...
class WindowsPrintConnectorTest extends PHPUnit_Framework_TestCase
{
    ...
    public function testSharedPrinterWindowsUsername()
    {
        ...
        $connector -> expects($this -> once())
                -> method('runCommand')
                -> with($this -> equalTo('net use \'\\\\example-pc\\Printer\' \'/site:bob\''));
        $connector -> expects($this -> exactly(0))
                -> method('runWrite');
        $connector -> expects($this -> once())
                -> method('runCopy')
    ...
    public function testSharedPrinterWindowsUsernameDomain()
    {
        ...
        $connector -> expects($this -> once())
                -> method('runCommand')
                -> with($this -> equalTo('net use \'\\\\example-pc\\Printer\' \'/site:home\\bob\''));
        $connector -> expects($this -> exactly(0))
                -> method('runWrite');
        $connector -> expects($this -> once())
                -> method('runCopy')
    ...
    public function testSharedPrinterWindowsUsernamePassword()
    {
        ...
        $connector -> expects($this -> once())
                -> method('runCommand')
                -> with($this -> equalTo('net use \'\\\\example-pc\\Printer\' \'/site:bob\' \'secret\''));
        $connector -> expects($this -> exactly(0))
                -> method('runWrite');
        $connector -> expects($this -> once())
                -> method('runCopy')
